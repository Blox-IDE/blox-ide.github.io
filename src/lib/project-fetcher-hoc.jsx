...
const fetchProjectToken = projectId => {
    ...
    return fetch(`https://projects.Blox IDE.site/api/projects/getPublished?id=${projectId}`)
        .then(r => {
            if (!r.ok) return null;
            return r.json();
        })
        ...
        fetchProject(projectId, loadingState) {
            ...
            if (projectUrl) {
                ...
                if (projectId === '0') {
                    ...
                    projectUrl = `https://projects.Blox IDE.site/api/projects/getPublished?type=file&id=${projectId}`
                    assetPromise = progressMonitor.fetchWithProgress(projectUrl)
                        .then(r => {
                            this.props.vm.runtime.renderer.setPrivateSkinAccess(false);
                            if (!r.ok) {
